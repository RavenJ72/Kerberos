public class DES {
    ...
    // S-box Table (таблица преобразований)
    private int[][][] sbox = {
            {
    ...
    private int[] shiftBits = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 }; //сдвиг

    private String stringToHex(String text){
        String hexString = "";
        byte[] bb = text.getBytes();
        for(byte x : bb){
            hexString += Integer.toHexString(x);
        }
        return hexString;
    }
    ...
    private String hextoBin(String input) {
        ...
        while (input.length() < n) // убрать 64 на n
            input = "0" + input;
        return input;
    }
    ...
    String decrypt(String plainText, String key) {
        ...
        return new String(hexStringToByteArray(plainText));
    }
}
